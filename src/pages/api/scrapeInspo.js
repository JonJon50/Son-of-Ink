export default async function handler(req, res) {
    ...
    try {
        ...
        try {
            ...
            // 🔁 If ALLOW_IG_ALL is true and no matches, randomly fallback 3 from Theron IG
            if (igResults.length === 0 && process.env.ALLOW_IG_ALL === 'true') {
                const all = (igData?.data || []).filter((item) => item.image);
                const shuffled = all.sort(() => 0.5 - Math.random());
                igResults = shuffled.slice(0, 3).map((ig) => ({
                    image: ig.image,
                    caption: `Instagram by theronissac: ${ig.caption || 'Tattoo inspiration'}`,
                    score: 0,
                    source: 'Instagram',
